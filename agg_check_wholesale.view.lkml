view: agg_check_wholesale {
  derived_table: {
    sql:
    select to_date(l.fulfilled) as DATE,
    ...
    from analytics.sales.sales_order s
                        ...
                        join     (select  a.date as date,
                                 ...
                                 group by 1) f on to_date(l.fulfilled) = to_date(f.date)
    where s.system = 'NETSUITE'
            and s.channel_id = 2
            and to_date(l.fulfilled) = dateadd(day,-2,current_date)
    group by 1;;
    }
