...
select  a.date as DATE,
        sum(case
               when a.EXTRACTED_SYSTEM = 'NETSUITE' then amount
               else 0
        end) as NETSUITE_AMOUNT,
        sum(case
               when a.EXTRACTED_SYSTEM = 'ANALYTICS' then amount
               else 0
        end) as ANALYTICS_AMOUNT,
        max(S.GROSS_SALES) as SHOPIFY_AMOUNT
from analytics.agg_check.DAILY_SOURCE_SALES_CHECK A join  (select   to_date(convert_timezone('America/Denver',s.CREATED_AT)) as CREATED,
