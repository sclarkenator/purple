view: incontact_phone {
  ...
  sql_table_name: "CUSTOMER_CARE"."CONTACT" ;;
  ...
  dimension: service_level_flag {
    label: "Service Level Flag"
    description: "Indicates whether the contact was handled within the skill defined Service Level, handled outside of the skill defined Service Level, or whether the contact never entered the queue."
    type: number
  ...
  dimension: out_sla {
    ...
    type: yesno
    sql: case when ${in_sla} = true then false
      else true end;;
  }
  ...
  measure: agent_offered_count {
    ...
    type: sum
    sql: case when ${agent_offered} = true then 1 else 0 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: hold_count {
    ...
    type: sum
    sql: case when ${holds} = true then 1 else 0 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: in_sla_count {
    ...
    type: sum
    sql: case when ${in_sla} = true then 1 else 0 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: inbound_count {
    ...
    type: sum
    sql: case when ${direction} = 'Inbound' then 1 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: outbound_count {
    ...
    type: sum
    sql: case when ${direction} = 'Outbound' then 1 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: transfer_count {
    ...
    type: sum
    sql: case when ${transferred} = true then 1 else 0 end ;;
    drill_fields: [detail*]
  }
  ...
  measure: acw_time_average {
    ...
    type: average
    value_format: "###0.00"
    sql: case when ${acw_time} > 0 then ${acw_time} end ;;
    drill_fields: [detail*]
  }

  measure: active_talk_time_average {
    label: "Active Talk Time AVG"
    group_label: "Average Measures"
  ...
  measure: long_abandon_pct {
    ...
    type: number
    value_format_name: percent_1
    sql: sum(case when ${long_abandon} = true then 1 else 0 end) / nullifzero(sum(case when ${agent_offered} = true then 1 else 0 end)) ;;
  ...
  measure: out_sla_pct {
    label:  "Out SLA Pct"
    group_label: "Percentage Measures"
    description: "Percent of queued calls that were NOT handled within the defined Service Level Objective."
    type: number
    value_format_name: percent_1
    sql: sum(case when ${out_sla} = true then 1 else 0 end) / nullifzero(sum(case when ${queued} = true then 1 else 0 end)) ;;
    drill_fields: [detail*, in_sla]
  }
  ...
  measure: Outbound_pct {
    ...
    type: number
    value_format_name: percent_1
    sql: sum(case when ${direction} = 'Outbound' then 1 end) / nullifzero(count(*)) ;;
    drill_fields: [detail*]
  }
}
