  ...
  dimension: week_bucket{
    ...
    type: string
     sql:  CASE WHEN date_trunc(week, ${TABLE}.forecast::date) = date_trunc(week, current_date) THEN 'Current Week'
             ...
             WHEN date_trunc(week, ${TABLE}.forecast::date) = date_trunc(week, dateadd(week, 0, dateadd(year, -1, current_date))) THEN 'Current Week LY'
             WHEN date_trunc(week, ${TABLE}.forecast::date) = date_trunc(week, dateadd(week, -1, dateadd(year, -1, current_date))) THEN 'Last Week LY'
             WHEN date_trunc(week, ${TABLE}.forecast::date) = date_trunc(week, dateadd(week, -2, dateadd(year, -1, current_date))) THEN 'Two Weeks Ago LY'
             ELSE 'Other' END ;; }
