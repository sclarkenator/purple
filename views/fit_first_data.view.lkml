...
view: fit_first_data {
  ...
  --where       date between '2021-10-01' and '2021-11-01'
...
, netsuite_refunds_raw as (
    ...
    left join   analytics_stage.nstl.transaction_lines        l       on l.transaction_id = t.transaction_id
    left join   analytics_stage.ns.transactions               t2      on t2.transaction_id = t.sales_order_ref_id
...
, first_data_raw as(
    ...
    select
                iff(cto.order_number is null,'Shopify','Commerce Tools') as platform,
                f.tag,
                f.amount,
    ...
    left join   analytics.commerce_tools.ct_order                   cto     on  f.ref_num = cto.order_number
    left join   analytics.commerce_tools.ct_order_payment           ctop    on  cto.order_id = ctop.order_id
    left join   analytics.commerce_tools.ct_payment                 ctp     on  ctop.payment_id = ctp.payment_id
...
,first_data_stage_g as (
    ...
    from        first_data_stage_f      f
    left join   netsuite_clean          n    on f.order_number = n.related_tranid
    ...
    left join   analytics_stage.shopify_us_ft.transaction           s       on f.ref_num = s.receipt:reference_no
                                                                            ...
                                                                            and f.tag::varchar = s.receipt:transaction_tag
    left join   analytics_stage.shopify_us_ft."ORDER"               o       on s.order_id = o.id
    left join   analytics_stage.ns.transactions                     t       on trim(coalesce(o.id::string,ctop.order_id)) = trim(t.etail_order_id)
                                                                            and t.transaction_type in ('Sales Order','Cash Sale')
                                                                            and coalesce(t.total_amount_ref,'0') <> '0'
    left join   analytics.sales.customer_deposit                    d       on t.transaction_id::string = d.sales_order_id::string
                                                                            and d.account = 'FIT - First Data'
    left join   analytics.sales.customer_deposit                    d2      on t.related_tranid::string = d2.related_tranid::string
