...
view: fit_first_data {
  derived_table: {
    ...
    with nt as (
      select
        regexp_replace(case
          when coalesce(t.memo,'') ilike '%DSC08%' then regexp_substr(coalesce(t.memo,''),'[Oo]?#?[ ]?N?[ ]?(\\d{4,7})',1,1,'e',1)
          when t.related_tranid is not null then t.related_tranid
          when t2.memo ilike '%DSC08%' then regexp_substr(t2.memo,'[Oo]?#?[ ]?N?[ ]?(\\d{4,7})',1,1,'e',1)
          when t2.related_tranid is not null then t2.related_tranid
          else null
      ...
      from analytics_stage.ns.transactions t
        ...
        left join analytics_stage.ns.transactions t2 on t.sales_order_ref_id = t2.transaction_id
      ...
      select
        f.tag,
        f.amount,
        iff(ct.interaction_id is null,f.ref_num,f.tag::string) as ref_num,
        f.auth_no,
        iff(ct.interaction_id is null,o.id::string,ct.order_id) as id,
        iff(ct.interaction_id is null,regexp_replace(o.name,'[ #N]{1,}',''),co.order_number) as order_number,
        f.time as created
      from analytics.accounting.first_data_transaction f
        ...
        left join analytics.commerce_tools.ct_transaction ct on f.tag::string = ct.interaction_id
        left join analytics.commerce_tools.ct_order co on ct.order_id = co.order_id
      ...
      select
        listagg(distinct f0.tag, ';') within group (order by f0.tag) as tag,
      ...
      from f
      left join n on f.order_number = n.related_tranid
      ...
      select
          ...
          iff(ct.interaction_id is null,f.ref_num,f.tag::string) as po_id,
    ...
    select
      ...
      abs(coalesce(transaction_amount,0) - coalesce(netsuite_amount,0)) as amount_difference, order_number, etail_order_id,
      netsuite_tran_id, netsuite_transaction_id, netsuite_customer_id
    from g
      ;;
  }
