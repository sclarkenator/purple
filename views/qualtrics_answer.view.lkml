view: qualtrics_answer {
  ...
  dimension: answer_order {
    ...
    type: number
    sql:
      case when ${TABLE}."ANSWER" in ('Extremely certain','Extremely helpful','Strongly agree','Extremely likely','1','Less than 1 week ago') then '1'
      when ${TABLE}."ANSWER" in ('Very certain','Very helpful','Somewhat agree','Somewhat likely','2','Less than 2 weeks ago') then '2'
      when ${TABLE}."ANSWER" in ('Moderately certain','Moderately helpful','Neither agree nor disagree','Neither likely nor unlikely','3','Less than 1 month ago') then '3'
      when ${TABLE}."ANSWER" in ('Slightly certain','Slightly helpful','Somewhat disagree','Somewhat unlikely','4','Less than 2 months ago') then '4'
      when ${TABLE}."ANSWER" in ('Not certain at all','Not helpful at all','Strongly disagree','Extremely unlikely','5','Less than 3 months ago') then '5'
      when ${TABLE}."ANSWER" in ('Less than 6 months ago') then '6'
      when ${TABLE}."ANSWER" in ('Less than 1 year ago') then '7'
      when ${TABLE}."ANSWER" in ('More than 1 year ago') then '8'
      else '99'
      end;;
  }
