...
view: daily_adspend {
  ...
  dimension_group: ad {
    ...
    label: "  Ad"
    type: time
    timeframes: [raw, date, day_of_week, day_of_month, day_of_year, week, month, month_name, quarter, quarter_of_year, year, week_of_year]
    sql: ${TABLE}.date ;; }
...
dimension: spend_platform {
  ...
  dimension: medium {
    ...
    case: {
      ...
      when: {sql: (${campaign_name} ilike '%native%' and not ${spend_platform} = 'ADMARKETPLACE') or ${campaign_name} ilike 'nt_%' or ${TABLE}.source in ('Native','NATIVE') OR ${TABLE}.platform in ('TABOOLA', 'MATTRESS TABOOLA');; label: "native" }
 ...
 dimension: ocean_bucket {
  ...
  dimension: product_focus{
    ...
    sql: case when campaign_name ilike '%acc%' then 'BEDDING'
                ...
                when campaign_name ilike '%illo%' then 'PILLOW'
                when campaign_name ilike '%armony%' then 'PILLOW'
