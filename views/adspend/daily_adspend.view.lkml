...
dimension: spend_platform {
  ...
  dimension: medium {
    ...
    case: {
      ...
      when: {sql: lower(${TABLE}.platform) in ('google','bing','verizon') and ${campaign_name} ilike ('%shopping%') ;; label: "pla"}
      ...
      when: {sql: ${TABLE}.platform in ('MADRIVO','ADWALLET','FKL', 'FLUENT','Fluent', 'LIVEINTENT', 'TALKABLE','KORTE','ROKT') ;; label: "crm" }
      else: "other" }
  }
