view: transaction_details {
  derived_table: {
    sql:
      ...
      join analytics_stage.ns.transactions on transactions.transaction_id = transaction_lines.transaction_id
      ...
      left join analytics_stage.ns.accounts on accounts.account_id = transaction_lines.account_id
      left join analytics_stage.ns.accounts as parent_account on parent_account.account_id = accounts.parent_id
      left join analytics_stage.ns.accounting_periods on accounting_periods.accounting_period_id = transactions.accounting_period_id
      left join analytics_stage.ns.income_accounts on income_accounts.income_account_id = accounts.account_id
      left join analytics_stage.ns.expense_accounts on expense_accounts.expense_account_id = accounts.account_id
      left join analytics_stage.ns.customers on customers.customer_id = transaction_lines.company_id
        and not customers._fivetran_deleted
      left join analytics_stage.ns.items on items.item_id = transaction_lines.item_id
        and not items._fivetran_deleted
      left join analytics_stage.ns.locations on locations.location_id = transaction_lines.location_id
      left join analytics_stage.ns.vendors on vendors.vendor_id = transaction_lines.company_id
        and not vendors._fivetran_deleted
      left join analytics_stage.ns.vendor_types on vendor_types.vendor_type_id = vendors.vendor_type_id
        and not vendor_types._fivetran_deleted
      left join analytics_stage.ns.currencies on currencies.currency_id = transactions.currency_id
