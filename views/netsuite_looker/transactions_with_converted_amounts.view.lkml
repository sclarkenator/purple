  ...
  derived_table: {
    sql:
      with period_exchange_rate_map as ( -- exchange rates used, by accounting period, to convert to parent subsidiary
        ...
        from analytics_stage.ns.consolidated_exchange_rates
        where consolidated_exchange_rates.to_subsidiary_id in (select subsidiary_id from analytics_stage.ns.subsidiaries where parent_id is null)  -- constrait - only the primary subsidiary has no parent
          and consolidated_exchange_rates.accounting_book_id in (select accounting_book_id from analytics_stage.ns.accounting_books where lower(is_primary) = 'yes')
        ...
        from analytics_stage.ns.accounts
        ...
        from analytics_stage.ns.accounting_periods as base
        join analytics_stage.ns.accounting_periods as multiplier
        ...
        where lower(base.quarter) = 'no'
          ...
          and base.fiscal_calendar_id = (select fiscal_calendar_id from analytics_stage.ns.subsidiaries where parent_id is null) -- fiscal calendar will align with parent subsidiary's default calendar
        group by 1
        ...
        join analytics_stage.ns.accounts on accounts.account_id = transactions_in_every_calculation_period_w_exchange_rates.account_id
       ;;
  }
}
