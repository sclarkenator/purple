view: shipping_times_for_web {
  ...
  derived_table: {
    sql:
with day_values as (select a.item_id, a.product_description, a.shipper, a.days, a.fulfilled_quantity, b.unfulfilled_quantity
        ...
        from (
            select item_id, product_description, days_to_fulfill days, shipper, sum(ordered_quantity) fulfilled_quantity
            from (
                select sol.order_id
                    , sol.system
                    , sol.item_id
            ...
            group by item_id, product_description, days_to_fulfill, shipper ) a

        full outer join (
        select item_id, product_description, days_since_order days, shipper, sum(ordered_quantity) unfulfilled_quantity
        from (
            select sol.order_id
                , sol.system
                , sol.item_id
        ...
        group by item_id, product_description, days_since_order, shipper

        ) b on b.item_id = a.item_id and b.product_description = a.product_description and b.days = a.days and b.shipper = a.shipper
        order by item_id, shipper, days )

select item_id, product_description, shipper fulfillment_location, Max(days) days_to_fulfill
from (
    select dv.*
    from day_values dv
...
) group by item_id, product_description, fulfillment_location
