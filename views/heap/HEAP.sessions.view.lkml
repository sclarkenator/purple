...
#   }
  ...
  dimension: medium_bucket {
    ...
    sql: case
          ...
          when ${utm_medium} = 'rf' and ${utm_term} ilike '%br%' then 'brand referral'
  ...
  dimension: source_bucket {
    ...
    sql: case
              ...
              when ${utm_source} in ('am', 'amazon media') then 'AMAZON MEDIA GROUP'
              ...
              when ${utm_source} in ('eb') then 'EBAY'
               when ${utm_source} in ('gb') then 'GEARBRAIN'
              ...
              when ${utm_source} ilike '%ob%' then 'OBE FITNESS'
